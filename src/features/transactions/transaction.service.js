...
async function createStripeCheckoutSessionForSubscription(userId, planId) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${config.FRONTEND_URL}/frontend/modules/payment/payment-success.html`,
    cancel_url: `${config.FRONTEND_URL}/frontend/modules/payment/payment-canceled.html`,
    metadata: {
      userId: userId,
      type: "subscription",
      planId: planId,
...
async function handleStripeWebhookEvent(event) {
  ...
  switch (event.type) {
      ...
      if (invoice.subscription) {
        ...
        if (userIdFromMetadata) {
          ...
          if (user && user.subscriptionType !== "PREMIUM") {
            await userRepo.update(userIdFromMetadata, {
              subscriptionType: "PREMIUM",
            });
          }
    ...
    case "customer.subscription.deleted": {
      ...
      await transactionRepo.updateManyByMetadata(
